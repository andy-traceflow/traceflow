...
**Update 2026-06-10:** both follow-ups landed. `jobs/monthly_report.py` ships the monthly report (confirmed vs estimated kept separate per the prime directive; ROI multiple against the new `revenue_config.monthly_fee`; program-to-date total anchored on lead creation date). GHL `fetch_recovered_value` is implemented as the **sum of the contact's won opportunities** — GHL has no `total_revenue` rollup, so the won-opportunity sum is this ADR's attribution unit for that provider; the decision-review trigger "a GHL client needs auto-sync" is satisfied pre-emptively since GHL is the default CRM. Monday still returns `None` (owner-report fallback).
