...
## 2026-07-23 — LLR conversation hotfix shipped (carve-out); CRM `external_id` de-overloaded (migration 026)

### milestone: missed-call → qualify → handoff works end-to-end in prod
Live re-test on `test-live-pilot` (clean slate): real missed call → greeting → four qualify turns (countertop / 89145 / marble / 100 sqft) → **one lead, no split**, deterministic `closing:v1` handoff, terminated `qualified` (score 65). The 2026-07-22 conversation fixes (NULL-`notes` 500, qualifier tool-use dead-end, deterministic closing) were **carved out of the ADR-0006 wave and shipped to `origin/main` ahead of it** (commit `6445d48`) — the onboarding/portal code stays local until it's finished — so those fixes are deployed while that wave is not. Verified against the prod event stream: `intent_classified` once, completeness 20→35→50→65, every turn `reply_sent: true`.

### fix (prod, P0): lead-splitting + turn budget — resume instead of re-interrogate (`6445d48`)
- **Resume, don't split.** `_process_sms_reply` looked up an open lead with `qualification_status IN ('unqualified','qualifying')` only, so any inbound after a *terminal* status opened a **new** lead (turn_count reset, intent gate re-run) and the caller got re-asked what they'd already answered. Now an inbound within a per-client window (`conversation_config.terminal_resume_window_minutes`, default 120) **resumes the same lead**: flip back to `qualifying`, keep captured fields, skip the intent gate, ask the next missing field. Window verdict computed in SQL (`now()` + `make_interval`) so it never couples the app clock to the DB clock; records a `lead_resumed` event. Outside the window → a genuinely new inquiry, unchanged.
- **Turn budget counts caller replies only.** `current_turn = inbound_turns` (from history) instead of `lead.turn_count + 1`; `turn_count` (both directions) stays the resume/activity counter. `max_turns: 8` now means 8 caller replies, not ~4 — **closes the known-gap flagged in the 2026-07-22 entry** (conversations no longer terminate at ~50% completeness for want of turns).

### fix (prod, P0): CRM auto-push silently skipped every missed-call lead (`3d5bee5`, migration 026)
`leads.external_id` was **overloaded** — documented as the CRM record id (migration 004) but seeded at creation with the **source-system id** (Twilio CallSid for missed calls; Shopify order id; generic field). Three CRM-id-keyed stages misread it: the auto-push guard `if external_id is not None: return` skipped **every** missed-call lead (found live 2026-07-23: qualified lead, no `crm_pushed` event, `external_id` held the CallSid, `pushed_to_crm_at` NULL); admin re-push PATCHed `/contacts/{CallSid}` (404); `revenue_sync` fetched recovered value with the CallSid → silent mismatch → **zero recovered-revenue attribution** for exactly the missed-call population. Fix: split the meanings — new **`leads.crm_external_id`** (mirrors `contacts.crm_external_id`) holds the CRM id, `external_id` keeps the source id, `pushed_to_crm_at` stays the pushed flag; all three consumers now key on `crm_external_id`. Backfill copies `external_id → crm_external_id` where `pushed_to_crm_at IS NOT NULL`. dedupe is unaffected (separate in-memory store; the CallSid also lives in `raw_payload`). The HubSpot Private-App `pat-` token is now the *only* remaining blocker for a full round trip — the push will log `crm_push_failed` on a bad token instead of skipping silently.

### ops: migration 026 applied to prod (ienjxmyhttuzxoaeramo) — schema head 025 → 026
`leads.crm_external_id` added (nullable) + backfilled (0 rows — no genuinely-pushed leads yet), verified live. Applied **before** the code deploy: unlike 025 (safe on defaults), the write-back needs the column. Note the two shipped commits are on `origin/main`, but the ADR-0006 onboarding/portal **code** (routers, middleware, provisioning, migration files 022–024) remains **uncommitted/local** — its DB migrations 022–025 are already on prod, so those columns/tables exist while the feature endpoints are not yet deployed.

### test: suite 555 passed / 50 skipped
+5 SMS-reply cases (resume within/outside window, intent-gate skip on resume, inbound-only budget, resumed lead qualifies) and the CRM regression (a missed-call lead with a CallSid in `external_id` and no `crm_external_id` now pushes); re-push and revenue-readback tests moved to `crm_external_id`.

### docs
Corrected the now-misleading `external_id = CRM id` references in `architecture.md` (Lead model + leads DDL), `workflow-schema.md` (crm_push stage), and the onboarding playbook checklist to distinguish `external_id` (source id) from `crm_external_id` (CRM id). Historical CHANGELOG entries and ADR-0003 left as point-in-time records.

---

## 2026-07-22 — Client-portal foundation, onboarding intake, and three prod conversation bugs (ADR-0006)

### fix (prod, P0): NULL `leads.notes` 500'd every inbound SMS reply
`Lead.notes` was declared `str = ""` against a **nullable** column with no default, and neither the missed-call nor the inbound-SMS insert sets it. Every such row read back as `None`, so `Lead(**dict(lead_row))` in `_process_sms_reply` raised `ValidationError` — a 500 on every SMS reply, so a real caller never got past the first qualification question. Fixed at both layers: migration **024** backfills the NULLs and sets `DEFAULT ''`, and a `mode="before"` validator on `Lead` coerces `None → ""` for rows written before that ran. Deliberately **not** widened to `str | None` — `lead.notes` is pushed straight into GHL/HubSpot/Monday payloads, so widening would leak a null into a client's CRM; and the column stays nullable because `LeadUpdate.notes=None` means "not provided" in a partial update. Audited the rest of the model: `notes` was the *only* nullable column among Lead's non-optional fields.

### fix (prod, P0): qualifier dead-ended when the model returned a tool call with no text
`run_qualifier_turn` made **one** Messages API call and harvested whatever blocks came back. When the model returns `stop_reason: "tool_use"` with only a `tool_use` block and no text — normal API behavior — `reply_text` was `""`, nothing was sent, and no termination fired: the conversation silently dead-ended mid-qualification (observed in prod right after the ZIP question, `reply_sent: false, termination: null`). Now runs the standard **tool-use continuation loop**: hand the `tool_result` back so the model writes its SMS. Two guards — it continues only while the reply is still empty (looping on a turn that already produced text would send the caller a **duplicate SMS**), and it's capped at `_MAX_TOOL_ROUNDS = 3`. Plus a loud `logger.warning` at the send site so an empty reply can never again look like a normal turn.

### build: conversations now resolve — code owns termination, so code owns the closing (migration 025)
`should_terminate` decided when a conversation ended, but nothing owned the **closing message**; whatever the model emitted on the terminal turn went out, in practice a bare `"Got it!"`. Worse, the model sometimes asks a follow-up question on that turn which the system will never process. `prompts/greeting.render_handoff` / `render_decline` now replace the model's terminal text with a deterministic ending: **handoff** (`qualified`/`needs_review`) tells the caller a real person will follow up — the product's core promise — while **decline** (hard-gate `disqualified`) deliberately does *not* promise a callback. Config-driven via `client_configs.handoff_template` / `decline_template` (nullable → defaults, `{business_name}` substitution, same convention as the migration-021 acks), editable in the admin Config panel. Closings are recorded with `ai_generated = false` and `prompt_version = 'closing:v1'` so they're distinguishable from model output in the transcript.

### build: portal auth keystone — resolve `client_id` from an authenticated principal (`middleware/portal_auth.py`)
The gap that would have forced a re-architecture later: ADR-0004 reserved Supabase Auth for the client portal, and `verify_jwt` / `require_permission` / `user_permissions` all existed — but **nothing derived `client_id` from a logged-in user** (tenant context came only from webhook URL paths), so `/api/kb` and `/api/calculator` returned zero rows or a 400 for any real user. `portal_principal` is a yielding dependency that verifies the Supabase JWT, loads memberships, resolves the active tenant, sets `app.current_client_id`, and clears it in a `finally` (same discipline as `tenant_resolver_middleware`). The membership lookup **bypasses RLS by necessity** — it's the query that decides the tenant scope — so it uses the service connection filtered explicitly by `user_id`. Multi-tenant users must name a tenant via `X-Client-Id`, validated against their memberships (403 on a non-member, never a silent "first tenant"). `admin_users` / `/api/admin` untouched — the two identity pools stay separate.

### build: onboarding staging + founder promote (migration 022) — no auto-provisioning
`onboarding_submissions` is **pre-tenant** data (no `client_id`), so it's service-role-locked like `admin_users`: RLS enabled + FORCED, zero policies. Form-agnostic (Jotform now, native form later): raw payload + a normalized `mapped_config` draft, with `promoted_client_id` `ON DELETE SET NULL` so off-boarding doesn't erase the record. `GET`/`PUT`/`POST …/promote` under `/api/admin/onboarding-submissions` (router-level `require_admin_user` + `forbid_demo_writes`, same as every other admin submodule); promote is idempotent (409 on re-promote) and atomic — client rows + the status flip commit together. Auto-provisioning on submit was rejected: a bad submission would mutate live tenant data with no review gate.

### build: one create-client path (`services/provisioning.py`), and a home for the orphan form fields (migration 023)
`provision_client` is now the **only** code that writes the `clients` + `client_configs` pair; the promote action and the refactored `scripts/onboard_client.py` both call it, so CLI and UI can't drift. Safe-write pattern from `update_client_config`: service connection, one transaction, `extra="forbid"` spec (a stray key is a validation error, never an injected column), no secrets. ~40% of the 77 Jotform fields had nowhere to land — address, DBA, website, GBP, structured owner/PoC contacts, tech inventory, logistics — so they get **one JSONB block**, `client_configs.business_profile`, rather than ~15 sparse columns, mirroring `brand`/`revenue_config` with accessors + defaults. `provision_portal_user` closes the loop: Supabase Auth user (invite/magic-link — no password touches us) + a `user_permissions` row with `is_admin = true`.

### build: admin-ui — Onboarding panel, closing-template editors, field-key autocomplete
`OnboardingPanel` (master/detail review, editable `mapped_config`, Promote/Save/Reject) is wired as a **top-level tab that renders with zero clients** — which is exactly when you need it. Config panel gains the two closing templates. The qualification editor's field `key` becomes a `<datalist>` autocomplete, **not** a `<select>`: `key` is a free-form config identifier (`models/qualification.py`), so locking it would break client-defined fields — the genuinely constrained neighbour, `maps_to`, was already a dropdown.

### ops: migrations 022–025 applied to prod (ienjxmyhttuzxoaeramo); test data reset
Applied via the Supabase MCP. Verified: `onboarding_submissions` created with RLS **enabled + forced + zero policies**; `business_profile` and the two closing templates present on `client_configs`; **0 NULL `leads.notes`** remaining with `DEFAULT ''` in place. Prod head 021 → **025**. Test-client (`test-live-pilot`) leads/messages/events/contacts wiped between live cycles — including `contacts`, so the caller is cold again and exercises the first-time path rather than the returning-caller resume window.

### infra: `app.traceflow.app` topology decided — no registrar migration
The working assumption was that serving `/admin` and `/manage` required moving the domain off Namecheap. Path routing is app-side (FastAPI mounts); the registrar is irrelevant. The real decision was **URL topology** — the app gets its own subdomain (Render custom domain + one Namecheap DNS record), marketing stays on `traceflow.app`. `TrustedHostMiddleware` added but **opt-in**: active only when `ALLOWED_HOSTS` is set, so existing deploys are unaffected until the app is actually exposed there.

### test: +23 tests; suite 549 passed / 50 skipped, ruff clean
Portal keystone (active-tenant resolution incl. 403 on a non-member tenant; DB-backed membership scoping + RLS proof), onboarding mapper/promote (mapping, `extra="forbid"`, promote creates rows + flips status, 409 re-promote, 422 on an unpromotable draft), portal user provisioning (mocked Supabase Admin API), the `Lead.notes` regression, the qualifier tool-use loop (the exact prod failure, `tool_result` echo, no-duplicate-SMS, round cap), the closings (handoff promises contact; **decline must not**), and the TrustedHost allow-list. Frontend type-checks and builds; Onboarding tab and both config fields verified rendering in the live demo with no console errors.

### known gaps (flagged, not built)
The `/manage` client SPA and the native onboarding form are **not** built — the Client-8 rule stands; this is the plumbing so that becomes UI-on-top work. Jotform stays as-is (no intake webhook wired yet — the staging table is ready for one). `provision_portal_user` isn't exercised against a real Supabase Admin API in CI. And `max_turns` compares against a `turn_count` that increments on **every message in both directions**, so `max_turns: 8` is ~4 caller replies — the reason a conversation can terminate at ~50% completeness. Left unchanged deliberately; raise it per client or change the comparison to count inbound turns as a separate decision.
